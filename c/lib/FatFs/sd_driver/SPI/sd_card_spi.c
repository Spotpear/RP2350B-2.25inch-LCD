 ...
 */
/*
 * Convert a 512-byte block (LBA) address into the argument expected by the
 * read/write commands. High-capacity cards (SDHC/SDXC == SDCARD_V2HC) use block
 * addressing, so the LBA is passed as-is. Standard-capacity cards (SDSC:
 * SDCARD_V1 / SDCARD_V2) use *byte* addressing, so the LBA must be multiplied
 * by the 512-byte block size. Block length is fixed to 512 via CMD16 at init.
 */
static inline uint32_t sd_addr_arg(sd_card_t *sd_card_p, uint32_t lba) {
    return (SDCARD_V2HC == sd_card_p->state.card_type)
               ? lba
               : (lba * (uint32_t)sd_block_size);
}
...
static block_dev_err_t in_sd_read_blocks(sd_card_t *sd_card_p, uint8_t *buffer,
    ...
    if (num_rd_blks == 1)
        status = sd_cmd(sd_card_p, CMD17_READ_SINGLE_BLOCK, sd_addr_arg(sd_card_p, data_address), false, 0);
    else
        status = sd_cmd(sd_card_p, CMD18_READ_MULTIPLE_BLOCK, sd_addr_arg(sd_card_p, data_address), false, 0);
    if (SD_BLOCK_DEVICE_ERROR_NONE != status) return status;
    ...
    do {
        ...
        if (status != SD_BLOCK_DEVICE_ERROR_NONE) {
            if (SD_BLOCK_DEVICE_ERROR_NONE !=
                ...
                /* NOTE: must not 'return' here - that would leak the SD mutex
                 * taken by sd_acquire() and make the NEXT read block forever in
                 * mutex_enter_blocking() (looks like a hang at f_mount). Break
                 * out so sd_release() below always runs. */
                                          ...
                                          uint32_t * const num_wrt_blks_p)
{
    ...
    status = sd_cmd(sd_card_p, CMD25_WRITE_MULTIPLE_BLOCK, sd_addr_arg(sd_card_p, *data_address_p), false, 0);
    if (SD_BLOCK_DEVICE_ERROR_NONE != status) return status;
                                          ...
                                          uint32_t const address)
{
    ...
    status = sd_cmd(sd_card_p, CMD24_WRITE_BLOCK, sd_addr_arg(sd_card_p, address), false, 0);
    if (SD_BLOCK_DEVICE_ERROR_NONE != status) return status;
...
DSTATUS sd_card_spi_init(sd_card_t *sd_card_p) {
    ...
    // SDSC (standard-capacity) cards are supported via byte addressing; see
    // sd_addr_arg(). Only reject cards that failed type detection entirely.
    if (SDCARD_NONE == sd_card_p->state.card_type ||
        CARD_UNKNOWN == sd_card_p->state.card_type) {
        EMSG_PRINTF("Unusable / undetected SD card\n");
        sd_release(sd_card_p);
        return sd_card_p->state.m_Status;
    }
