...
/* Partition base offset (in 512-byte sectors) added to every LBA. Used to
 * present a single partition (e.g. a FAT volume inside a GPT disk) to FatFs as
 * a "super-floppy" so its VBR appears at logical sector 0. 0 = no offset
 * (normal MBR/SFD cards, handled by FatFs itself). Set before f_mount. */
uint32_t g_sd_lba_offset = 0;
...
DRESULT disk_read(BYTE pdrv,  /* Physical drive number to identify the drive */
    ...
    sd_card_t *sd_card_p = sd_get_by_num(pdrv);
    if (!sd_card_p) return RES_PARERR;
    int rc = sd_card_p->read_blocks(sd_card_p, buff, sector + g_sd_lba_offset, count);
...
DRESULT disk_write(BYTE pdrv, /* Physical drive number to identify the drive */
    ...
    sd_card_t *sd_card_p = sd_get_by_num(pdrv);
    if (!sd_card_p) return RES_PARERR;
    int rc = sd_card_p->write_blocks(sd_card_p, buff, sector + g_sd_lba_offset, count);
    return sdrc2dresult(rc);
}
