...
void Paint_Clear(UWORD Color)
{
    if(Paint.Scale == 2 || Paint.Scale == 4) {
        ...
        /* WidthByte is already WidthMemory*2 (bytes per row). Iterate over
         * PIXELS (WidthMemory) and address at X*2, otherwise the old
         * `X < WidthByte` combined with `X*2` walks ~2x the row width and
         * writes past the end of the framebuffer every clear, corrupting the
         * heap just after it (crash/black screen once RAM layout shifts). */
        for (UWORD Y = 0; Y < Paint.HeightByte; Y++) {
                         ...
                         sFONT* Font, UWORD Color_Foreground, UWORD Color_Background)
{
    ...
    while (* pString != '\0') {
        ...
        /* NOTE: pass (Color_Foreground, Color_Background) in the correct order.
         * The old swapped call (Color_Background, Color_Foreground) made the
         * FONT_BACKGROUND fast-path draw glyphs in the screen's background
         * color whenever the caller's foreground was WHITE -> text invisible
         * (solid blue "0 photos" screen, solid red error screens). */
                   ...
                   sFONT* Font, UWORD Digit,UWORD Color_Foreground, UWORD Color_Background)
{
    ...
    //show (pass colors in the same foreground/background order as this API)
